...
import scalation.columnar_db.TableObj._
...
//      else if (r2.col(0) == null) return this
        ...
        val newCol = for (j <- col.indices.par) yield Vec.++ (col(j), r2.columns(j))
    ...
    def intersect (_r2: Table): Relation =
    {
        ...
        for (i <- orderedIndex.indices.par) {
    ...
    def intersect2 (r2: Table): Relation =
    {
        ...
        for (i <- (0 until rows).par if r2 contains row(i)) r3.add (row(i))
        r3.materialize ()
    } // intersect2
    ...
    def join [T] (_r2: Table, p0: Predicate2 [T], p: Predicate2 [T]*): Relation =
    {
        ...
        for (i <- (smallmapbig.keySet.toVector.sorted).par) {
...
// runMain scalation.columnar_db.RelationTest11
object RelationTest11 extends App
{
    /*var r1 = Relation("test", 0, "SID", "c1", "c2", "c3")
//    r.show()
    var r: Array[Relation] = Array()
    r = r :+ r1
    println(r.length)
    r(0).show()
    ...
    if (professor.col == professor2.col) println("Same")
    else println("Not same")

 //   professor.select[Int]("title", x => x < 3).show()
}

/*
// runMain scalation.columnar_db.RelationTest12
object RelationTest12 extends App
{
    val professor = Relation ("professor", 0, "ISS", "pid", "name", "prodeptid")
    TableGen.popTable (professor, 2000)

    val courses = Relation ("courses", 0, "IISS", "cid", "pid", "cname", "descr")
    TableGen.popTable (courses, 2000)

    val noOfIter = 5

    professor.join(courses)
    ...
    professor.join_par(professor.colNames intersect courses.colNames, courses)
    ...
    professor.join_par2(professor.colNames intersect courses.colNames, courses)
    ...
    println("Normal: " + (totTime / 1000000000.0) + "s")
    println("par1: " + (totTime1 / 1000000000.0) + "s")
    println("par2: " + (totTime2 / 1000000000.0) + "s")
